...
def test_after_find_called_for_base_model_finders():
    ...
    assert found.found_name == "found:finder"
...
def test_after_find_called_for_query_wrapper_methods():
    ...
    assert found is not None
    assert found.found_name == "found:first"
    assert events == ["after_find:first"]

    events.clear()
    found = AfterFindModel.select().first()
    assert found is not None
    assert found.found_name == f"found:{found.name}"
    ...
    assert len(found) == 2
    assert {record.found_name for record in found} == {"found:first", "found:second"}
...
def test_after_find_called_for_find_or_initialize_existing_only():
    ...
    assert existing.found_name == "found:existing"
    ...
    assert new_instance.found_name is None
    assert events == []
