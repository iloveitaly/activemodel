...
Follow `.cursor/skills/upgrade-sqlmodel/SKILL.md`.

That skill covers the `sqlmodel` and `typeid-python` pins, the copied `get_column_from_field` patch, the `get_sqlalchemy_type` wrapper, and the SQLModel changes that live outside those two functions. Do not commit.
